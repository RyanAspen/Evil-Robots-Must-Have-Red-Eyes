...
public class BugNav extends Globals {
    ...
    private static void moveWithFill(Direction direction) throws GameActionException {
        ...
        {
            case SOLDIER:
                if (!rc.senseMapInfo(fillLocation).getPaint().isAlly())
                {
                    if (rc.canAttack(fillLocation) && rc.getPaint() > 15)
                    {
                        PaintManager.soldierAttack(fillLocation);
                        rc.move(direction);
                        canMove = false;
                    }
                    else
            ...
            case SPLASHER:
                if (!rc.senseMapInfo(fillLocation).getPaint().isAlly())
                {
                    if (rc.canAttack(fillLocation) && rc.getPaint() > 75)
                    {
                        if (rc.senseMapInfo(fillLocation).getMark().isSecondary())
                        {
                            rc.attack(fillLocation, true);
                        }
                        else {
                            rc.attack(fillLocation);
                        }
                        rc.move(direction);
                        canMove = false;
                    }
                ...
                break;
            case MOPPER:
                if (rc.senseMapInfo(fillLocation).getPaint() != PaintType.EMPTY && !rc.senseMapInfo(fillLocation).getPaint().isAlly())
                {
                    return;
                }
                else
