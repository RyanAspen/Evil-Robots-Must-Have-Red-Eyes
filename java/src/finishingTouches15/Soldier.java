...
public class Soldier extends Globals {
    ...
    public static void paintRuins() throws GameActionException {
        if (rc.getPaint() <= UnitType.SOLDIER.attackCost || rc.getNumberTowers() >= GameConstants.MAX_NUMBER_OF_TOWERS)
        {
            return;
        }
        ...
        if (targetRuin == null)
        {
            MapLocation closestRuin = KnowledgeBase.getClosestRuin();
            if (closestRuin != null)
            {
                Nav.move(closestRuin);
                return;
            }
        }
        ...
        if (rc.getPaint() >= GameConstants.MARK_PATTERN_PAINT_COST+10 && targetRuin != null && isTargetRuinMarkable)
        {
            ...
            if (rc.getRoundNum() < Math.max(mapHeight,mapWidth)) //if (rc.getRoundNum() < 60)
            {
            ...
            /*
            //If we don't know of a paint tower, make one
            if (KnowledgeBase.getClosestAllyPaintTower() == null)
            {
            ...
            double paintProb = 0.8;
            double moneyProb = 1 - paintProb;
            if (rng.nextDouble(1) < moneyProb && rc.getMoney() < 2000)
            {
                if (rc.canMarkTowerPattern(UnitType.LEVEL_ONE_MONEY_TOWER, targetRuin)) {
                    Logger.addToIndicatorString("Marking TP " + targetRuin);
                    rc.markTowerPattern(UnitType.LEVEL_ONE_MONEY_TOWER, targetRuin);
