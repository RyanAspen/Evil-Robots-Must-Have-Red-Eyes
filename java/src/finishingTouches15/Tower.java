...
public class Tower extends Globals
{
    ...
    /*
        If we see enemy paint and no allied moppers, spawn a mopper
        close to the enemy paint.
     */
    public static void attemptToClearUpNearby() throws GameActionException {
        //Check for enemy paint.
        MapInfo[] nearInfos = rc.senseNearbyMapInfos();
        MapLocation nearestEnemyPaint = null;
        int minDist = 10000;
        for (int i = 0; i < nearInfos.length; i++)
        {
            if (nearInfos[i].getPaint().isEnemy())
            {
                int dist = rc.getLocation().distanceSquaredTo(nearInfos[i].getMapLocation());
                if (dist < minDist)
                {
                    minDist = dist;
                    nearestEnemyPaint = nearInfos[i].getMapLocation();
                }
            }
        }
        ...
        {
            //Now check for nearby allied moppers
            RobotInfo[] nearAllies = rc.senseNearbyRobots(-1, myTeam);
            for (int i = 0; i < nearAllies.length; i++)
            {
                if (nearAllies[i].getType() == UnitType.MOPPER) return;
            }

            //Try to spawn a mopper
    ...
    public static void run() throws GameActionException {
        ...
        attemptToClearUpNearby();

        //If we have a TON of gold and no enemies are nearby, destroy this tower
        if (rc.getMoney() > 10000 && rc.getPaint() < 100 && roundsSinceEnemySeen > 200)
        {
            if (Micro.towerPatternToComplete(rc.getLocation(), true) != null) {
                RobotInfo[] nearAllies = rc.senseNearbyRobots(10, myTeam);
                boolean found = false;
                for (int i = 0; i < nearAllies.length; i++) {
