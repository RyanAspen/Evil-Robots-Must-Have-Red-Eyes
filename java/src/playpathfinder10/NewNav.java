...
public class NewNav extends Globals{
    ...
    static boolean isAllyAdjacent = false;
    static boolean isNeutralAdjacent = false;
    ...
    public static void updatePaintAdjacencies() throws GameActionException {
        isAllyAdjacent = false;
        isNeutralAdjacent = false;
        for (int i = 0; i < allDirections.length; i++)
        {
            MapLocation loc = rc.getLocation().add(allDirections[i]);
            if (!rc.canSenseLocation(loc)) continue;
            MapInfo info = rc.senseMapInfo(loc);
            if (info.getPaint().isAlly()) isAllyAdjacent = true;
            if (info.getPaint() == PaintType.EMPTY) isNeutralAdjacent = true;
        }
    }
    ...
    public static void bug2(MapLocation target) throws GameActionException {
        ...
        updatePaintAdjacencies();
        boolean dirWall = false;
        boolean dirWait = false;
        ...
        careful = rc.getPaint() < 30;

        if (((rc.getRoundNum() < 100 && !careful) || turnsSinceMovement > 5 || currLoc.isWithinDistanceSquared(target, GameConstants.VISION_RADIUS_SQUARED)) && turnsOfAggroMovement == 0)
        {
    ...
    public static boolean isWall(Direction d) throws GameActionException {
        ...
        if (careful)
        {
            if (paint == PaintType.EMPTY && isAllyAdjacent) return true;
            if (paint.isEnemy() && (isNeutralAdjacent || isAllyAdjacent)) return true;
        }
