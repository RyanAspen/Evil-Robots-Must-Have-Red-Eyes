...
public class Tower extends Globals
{
    ...
    public static void replaceWith(UnitType type) throws GameActionException {
        ...
        for (int x2 = 0; x2 < 5; x2++)
        {
            for (int y2 = 0; y2 < 5; y2++)
            {
                ...
                switch (rc.senseMapInfo(new MapLocation(x+x2-2,y-y2+2)).getPaint())
                {
                    case ALLY_PRIMARY:
                        if (pattern[y2][x2]) {
                ...
                if (!canDisintegrate) break;
            }
            if (!canDisintegrate) break;
        }
        if (canDisintegrate) rc.disintegrate();
    }
    ...
    public static void run() throws GameActionException {
        ...
        if ((rc.getType() == UnitType.LEVEL_ONE_MONEY_TOWER || rc.getType() == UnitType.LEVEL_TWO_MONEY_TOWER || rc.getType() == UnitType.LEVEL_THREE_MONEY_TOWER) && rc.getMoney() > 3000 && rc.getNumberTowers() > 20 && rc.senseNearbyRobots(-1,opponentTeam).length == 0) rc.disintegrate();//replaceWith(UnitType.LEVEL_ONE_DEFENSE_TOWER);
