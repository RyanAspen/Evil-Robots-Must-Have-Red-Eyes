...
public class Micro extends Globals {
    ...
    public static MapLocation checkForWeakenedEnemyTower() throws GameActionException {
        ...
        for (int i = 0; i < enemies.length; i++)
        {
            ...
            if (towerPatternToComplete(enemies[i].getLocation(), false) != null) continue;
    ...
    public static void soldierMicro() throws GameActionException {
        ...
        MapLocation towerTarget = checkForWeakenedEnemyTower();
        if (towerTarget != null) {
            //If we aren't already in attack range, move into attack range
            if (rc.getLocation().distanceSquaredTo(towerTarget) > UnitType.SOLDIER.actionRadiusSquared) {
                Nav.move(towerTarget);
            }

            //If our current location has empty paint, paint it.
            if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
                ...
                if (rc.canAttack(towerTarget)) {
                    rc.attack(towerTarget);
                }
            }
            return;
        }
