...
public class MovementMicro{
    ...
    /*
        TODO:
            If a soldier sees an enemy tower and its pattern is disrupted,
            ignore all other micro and move within attack range, preferring lower
            paint loss.
    ...
    public void preprocessMicroSoldier() throws GameActionException {
        ...
        boolean canAttack = rc.isActionReady() && rc.getPaint() > rc.getType().attackCost*5 && rc.getHealth() > 40;
    ...
    public void preprocessMicroSplasher() throws GameActionException {
        ...
        for (int i = 0; i < allDirections.length; i++)
        {
            ...
            {
                type = rc.senseMapInfo(loc).getPaint();
            }
            microInfoSplashers[i] = new MicroInfoSplasher(loc, allDirections[i], AdjacencyManager.getPaintLoss(allDirections[i]),type,rc.canMove(allDirections[i]));
        }
